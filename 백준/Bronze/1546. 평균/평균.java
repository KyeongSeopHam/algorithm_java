...
public class Main {
    public static void main(String[] args) {
        ...
        Scanner sc= new Scanner(System.in);    // 갯수

        double sum = 0 ;
        double score[] = new double[sc.nextInt()];  // 갯수만큼 double type arr init

        for(int i=0; i<score.length; i++){
            score[i] = sc.nextDouble(); // 성적 입력 (double)
        }

        /** 성적이 큰 순서를 먼저 뽑아서 기준이 되야함
          일단 세준이는 자기 점수 중에 최댓값을 골랐다.
         이 값을 M이라고 한다. 그리고 나서 모든 점수를 점수/M*100으로 고쳤다.
        */
        Arrays.sort(score);  // asc sort
